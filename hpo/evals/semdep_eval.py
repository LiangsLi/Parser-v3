...
def compute_F1(gold_file, sys_file, labeled=False):
    ...
    with codecs.open(gold_file, encoding='utf-8') as gf,\
        ...
        while gold_line:
            ...
            if gold_line.rstrip() != '':
                ...
                while sys_line.rstrip() == '' or sys_line.split('\t')[
                        0] == '0':
                    sys_line = sf.readline()
                ...
                if gold_node != '_':
                    ...
                    if labeled:
                        gold_edges = set(tuple(gold_edge.split(':', 1))
                                         for gold_edge in gold_node)
                    else:
                        gold_edges = set(
                            gold_edge.split(
                                ':', 1)[0] for gold_edge in gold_node)
                else:
                    gold_edges = set()
                ...
                if sys_node != '_':
                    ...
                    if labeled:
                        sys_edges = set(tuple(sys_edge.split(':', 1))
                                        for sys_edge in sys_node)
                    else:
                        sys_edges = set(
                            sys_edge.split(
                                ':', 1)[0] for sys_edge in sys_node)
                else:
                    sys_edges = set()
    ...
    Accuracy = namedtuple(
        'Accuracy', [
            'precision', 'recall', 'F1', 'tok_acc', 'seq_acc'])
...
def semdep_cleanup(hyperparam_dict):
    ...
    if not hyperparam_dict['SemrelGraphTokenVocab']['factorized']:
        ...
        if hyperparam_dict['SemrelGraphTokenVocab']['decomposition_level'] in (
            '0',
                0):
        ...
        if hyperparam_dict['SemheadGraphIndexVocab']['decomposition_level'] in (
            '0',
                0):
    ...
    if hyperparam_dict['SemrelGraphTokenVocab']['decomposition_level'] in (
        '0',
            0):
        hyperparam_dict['Network']['recur_size'] = np.clip(
            hyperparam_dict['Network']['recur_size'], 10, 450)
