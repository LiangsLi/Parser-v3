...
def evaluate_tokens(gold_file, sys_file, labeled=False, force=False):
    ...
    with codecs.open(gold_file, encoding='utf-8') as gf,\
        ...
        while gold_line:
            while gold_line.startswith('#') or re.match(
                    r'[0-9]*\.', gold_line):
                if current_n_tokens:
            ...
            if gold_line != '\n':
                ...
                # Accumulate
                if labeled:
                    current_n_correct_tokens += (gold_head ==
                                                 sys_head) and (gold_label == sys_label)
                else:
...
if __name__ == '__main__':
    ...
    print(
        evaluate_tokens(
            'data/test/gold/conll17-ud-test-2017-05-09/en.conllu',
            'saves/test/parsed/data/test/gold/conll17-ud-test-2017-05-09/en.conllu'))
