...
class BaseHyperparam(object):
    ...
    def plot(self, scores):
        ...
        mean = weights.dot(x)
        centered = x - mean
        var = centered.dot(np.diag(weights)).dot(
            centered) / (1 - weights.dot(weights))
        ...
        ax.scatter([self.denormalize(value)
                    for value in self.values], scores, alpha=.5, edgecolor='k')
