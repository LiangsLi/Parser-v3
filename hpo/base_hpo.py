...
class BaseHPO(list):
        ...
        for row in rand_df.itertuples():
            if row.bounds.startswith('%'):
                ...
                while copyrow.bounds.startswith('%'):
                    ...
                    match = re.match(r'%\((\w+):(\w+)\)', copyrow.bounds)
                    section = match.group(1)
                    option = match.group(2)
                    copyrow = rand_df.loc[(rand_df.section == section) & (
                        rand_df.option == option)].iloc[0]
                name2hp[section + ':' +
                        option].add_copy(row.section, row.option)
    ...
    def improve_rank(self, dims=None):
        ...
        hps = [
            hp for i,
            hp in enumerate(self) if (
                not hp.fixed) and (
                (dims is None) or (
                    i in dims))]
        d = len(hps)
        discretes = [hp for hp in hps if isinstance(
            hp, (BoolHyperparam, StringHyperparam))]

        discrete_values = {}
        for discrete in discretes:
            for i, bound in enumerate(discrete.bounds):
                discrete_values[(discrete.name, bound)] = np.equal(
                    discrete.values, i).astype(int)
        ...
        if dims is not None or hps_to_remove:
            dims = [
                i for i, hp in enumerate(self) if (
                    (i in dims) and (
                        (hp.section, hp.option) not in hps_to_remove))]
    ...
    def plots(self):
        ...
        axt = ax.twiny()
        axt.plot(dist, x)
        axt.fill_between([np.min(dist)] +
                         list(dist) +
                         [np.min(dist)], [np.max(x)] +
                         list(x) +
                         [np.min(x)], alpha=.25)
        pretty_plot(ax)
        fig.tight_layout()
...
if __name__ == '__main__':
    ...
    def eval_func(save_dir):
        ...
        return evals.evaluate_tokens(
            base, os.path.join(
                save_dir, 'parsed', base), force=False)
    # -------------------------------------------------------------
