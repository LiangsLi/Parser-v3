    ...
    if network_list in ('None', ''):
        ...
        if network_class not in networks:
            for _network_class in network_list:
                ...
                input_networks, networks = resolve_network_dependencies(_config, _network_class, _network_list,
                                                                        networks)
...
def main():
    ...
    argparser.add_argument('--save_dir')  # saves/sem16-v0
...
def run(**kwargs):
    ...
    print('network class:', str(network_class))
    network_list = config.get(network_class, 'input_network_classes')
    print('network list:',str(network_list))
    input_networks, networks = resolve_network_dependencies(config, network_class, network_list, {})
    print("input networks:",str(input_networks))
    print("networks:",str(networks))
    NetworkClass = getattr(parser, network_class)
    print('NetworkClass:',str(NetworkClass))
    print('----------------------config---------------------')
    pprint(config)
    print('-------------------------------------------------\n')
    network = NetworkClass(input_networks=input_networks, config=config)
