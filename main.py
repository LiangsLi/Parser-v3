...
# -------------------------------------------------------------
def resolve_network_dependencies(
        config,
        network_class,
        network_list,
        networks):
    if network_list in ('None', ''):
        ...
        if network_class not in networks:
            for _network_class in network_list:
                ...
                _network_list = _config.get(
                    _network_class, 'input_network_classes')
                input_networks, networks = resolve_network_dependencies(
                    _config, _network_class, _network_list, networks)
                NetworkClass = getattr(parser, _network_class)
                networks[_network_class] = NetworkClass(
                    input_networks=input_networks, config=config)
        return set(networks[_network_class]
                   for _network_class in network_list), networks
...
def main():
    ...
    argparser.add_argument(
        '--save_dir',
        default="saves/sem16-v0",
        help="TF's model checkpoint file")
...
def train(**kwargs):
    ...
    kwargs = {
        key: value for key,
                       value in six.iteritems(kwargs) if value is not None}
    ...
    if save_dir is None:
        save_dir = Config(config_file=config_file, **
        kwargs).get('DEFAULT', 'save_dir')
    ...
    if not load and os.path.isdir(save_dir):
        if not force:
            ...
            while input_str not in ('y', 'n', 'yes', 'no'):
                input_str = input(
                    '{} already exists. It will be deleted if you continue. Do you want to proceed? [Y/n] '.format(
                        save_dir)).lower()
...
def hpo(**kwargs):
    ...
    base = 'data/CoNLL18/UD_{}-{}/{}_{}-ud-dev.conllu'.format(
        lang, treebank, lc, tb)
...
def run(**kwargs):
    ...
    network_class = config.get(
        'DEFAULT', 'network_class')  # GraphParserNetwork
    network_list = config.get(network_class, 'input_network_classes')  # None
    input_networks, networks = resolve_network_dependencies(
        config, network_class, network_list, {})  # set(), {}
    NetworkClass = getattr(parser, network_class)   # GraphParserNetwork class
    network = NetworkClass(input_networks=input_networks, config=config)    # GraphParserNetwork
    network.parse(
        conllu_files,
