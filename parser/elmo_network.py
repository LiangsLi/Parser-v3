...
class ElmoNetwork(BaseNetwork):
    ...
    def build_graph(self, input_network_outputs={}, reuse=True):
        ...
        with tf.variable_scope('Embeddings'):
            input_tensors = [
                input_vocab.get_input_tensor(
                    reuse=reuse) for input_vocab in self.input_vocabs]
            for input_network, output in input_network_outputs:
                with tf.variable_scope(input_network.classname):
                    input_tensors.append(
                        input_network.get_input_tensor(
                            output, reuse=reuse))
        ...
        token_weights = nn.greater(
            self.id_vocab.placeholder, 0, dtype=tf.int32)
        ...
        with tf.variable_scope('RNN_FW-{}/RNN/Loop'.format(i), reuse=True):
            ...
            n_splits = fw_initial_state.get_shape(
            ).as_list()[-1] / self.recur_size
        ...
        if self.bidirectional:
            with tf.variable_scope('RNN_BW-{}/RNN/Loop'.format(i), reuse=True):
                ...
                n_splits = bw_initial_state.get_shape(
                ).as_list()[-1] / self.recur_size
                bw_initial_state = tf.split(
                    bw_initial_state, int(n_splits), -1)[0]
                stop_token = ones * bw_initial_state
                rev_layer = tf.concat([stop_token, layer], axis=1)
                rev_layer = tf.reverse_sequence(
                    rev_layer, seq_lengths + 1, seq_axis=2)[:, 1:]
            if self.bilin:
                layer = tf.concat(
                    [layer * rev_layer, layer, rev_layer], axis=2)
            else:
