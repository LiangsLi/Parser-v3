...
class FeatureVocab(BaseVocab):
    ...
    # =============================================================
    # TODO fix this to compute feature-level F1 rather than token-level
    # accuracy
    def get_linear_classifier(
        ...
        with tf.variable_scope(variable_scope or self.classname):
            ...
            with tf.variable_scope('Classifier'):
                ...
                for i, feat in enumerate(self._feats):
                    ...
                    with tf.variable_scope(vs_feat):
                        logits = classifiers.linear_classifier(
                            layer, self.getlen(feat), hidden_keep_prob=hidden_keep_prob)
                ...
                # (n x m x f) -> (n x m)
                correct_tokens = tf.reduce_prod(
                    correct_tokens, axis=-1) * token_weights
                # (n x m) -> (n)
        ...
        with tf.variable_scope(variable_scope or self.classname):
            ...
            with tf.variable_scope('Classifier'):
                ...
                for i, feat in enumerate(self._feats):
                    ...
                    with tf.variable_scope(vs_feat):
                        if self.diagonal:
                            logits = classifiers.diagonal_bilinear_classifier(layer1, layer2, self.getlen(
                                feat), hidden_keep_prob=hidden_keep_prob, add_linear=self.add_linear)
                        else:
                            logits = classifiers.bilinear_classifier(layer1, layer2, self.getlen(
                                feat), hidden_keep_prob=hidden_keep_prob, add_linear=self.add_linear)
                        targets = self.placeholder[:, :, i]
            ...
            with codecs.open(train_conllu, encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        ...
                        multitoken = line[self.conllu_idx]
    ...
    def index(self, multitoken):
        ...
        assert isinstance(
            multitoken, six.string_types), 'FeatureVocab.index was passed {}'.format(multitoken)
    ...
    def get_root(self):
        ...
        return 'Root=Yes' if self.keyed else self.separator.join(
            [self.ROOT_STR for _ in self._feats])
    ...
    def __getitem__(self, key):
        assert hasattr(
            key, '__iter__'), 'You gave FeatureVocab.__getitem__ {}'.format(key)
        if isinstance(key, six.string_types):
            ...
            if self.keyed:
                ...
                return [
                    self._str2idx.get(
                        feat,
                        self._str2idx['Root']).get(
                        key_dict[feat],
                        self.UNK_IDX) if feat in key_dict else self.PAD_IDX for feat in self._feats]
            else:
                if self.cliplen:
                    multitoken = multitoken[:self.cliplen]
                return [self._str2idx[str(feat)].get(
                    key, self.UNK_IDX) for feat, key in enumerate(multitoken)]
        elif isinstance(key[0], six.integer_types + (np.int32, np.int64)):
            if self.keyed:
                multitoken = [
                    '{}={}'.format(
                        feat, self._idx2str.get(
                            feat, self._idx2str['Root']).get(
                            key, self.UNK_STR)) for feat, key in zip(
                        self._feats, key) if key != self.PAD_IDX]
            else:
                multitoken = [self._idx2str[str(feat)].get(
                    key, self.UNK_STR) for feat, key in enumerate(key)]
            if np.sum(key) > 0:
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(key[1], six.string_types):
            ...
            raise ValueError(
                'keys and values to {}.__setitem__ must be (iterables of) (string, string or integer) tuples and string or integer')
    ...
    def __contains__(self, key):
        ...
        if isinstance(key, six.string_types):
            ...
            raise ValueError(
                'key to {}.__contains__ must be (string, string or integer) tuple')
