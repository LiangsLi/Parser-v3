...
class IndexVocab(BaseVocab):
        ...
        with tf.variable_scope(variable_scope or self.field):
            ...
            with tf.variable_scope('Attention'):
                ...
                if linearize:
                    ...
                    lin_xent = - \
                        tf.nn.softplus(tf.where(lin_ids, -lin_logits, lin_logits))
                ...
                if distance:
                    ...
                    dist_ids = tf.to_float(
                        tf.tile(tf.abs(head_ids - dep_ids), [batch_size, 1, 1])) + 1e-12
                    # (n x m x m), (n x m x m) -> (n x m x m)
                ...
                non_pads = tf.to_float(token_weights) + tf.to_float(
                    tf.logical_not(tf.cast(tf.range(bucket_size), dtype=tf.bool)))
                # (n x m x m) o (n x 1 x m) -> (n x m x m)
                probabilities = tf.nn.softmax(
                    logits) * tf.expand_dims(non_pads, -2)
                ...
                if linearize:
                    ...
                    lin_target_xent = tf.matmul(
                        lin_xent_reshaped, one_hot_targets)
                    # (n x m x 1 x 1) -> (n x m)
                    lin_target_xent = tf.squeeze(lin_target_xent, [-1, -2])
                    # (n x m), (n x m), (n x m) -> ()
                    loss -= tf.reduce_sum(lin_target_xent * \
                                          tf.to_float(token_weights)) / (n_tokens + 1e-12)
                if distance:
                    # (n x m x m) -> (n x m x 1 x m)
                    dist_kld_reshaped = tf.expand_dims(dist_kld, -2)
                    # (n x m x 1 x m) * (n x m x m x 1) -> (n x m x 1 x 1)
                    dist_target_kld = tf.matmul(
                        dist_kld_reshaped, one_hot_targets)
                    # (n x m x 1 x 1) -> (n x m)
                    dist_target_kld = tf.squeeze(dist_target_kld, [-1, -2])
                    # (n x m), (n x m), (n x m) -> ()
                    loss -= tf.reduce_sum(dist_target_kld * \
                                          tf.to_float(token_weights)) / (n_tokens + 1e-12)
    ...
    def __getitem__(self, key):
        if isinstance(key, six.string_types):
            ...
            raise ValueError(
                'key to IndexVocab.__getitem__ must be (iterable of) string or integer')
        return
    # =============================================================
    @property
    def distance(self):
...
class GraphIndexVocab(IndexVocab):
        ...
        with tf.variable_scope(variable_scope or self.field):
            ...
            with tf.variable_scope('Discriminator'):
                ...
                probabilities = tf.nn.sigmoid(
                    logits) * tf.to_float(token_weights)
                # (n x m x m), (n x m x m), (n x m x m) -> ()
    ...
    def __getitem__(self, key):
        if isinstance(key, six.string_types):
            ...
            if len(key) > 0:
                if isinstance(key[0], six.integer_types +
                              (np.int32, np.int64)):
            ...
            raise ValueError(
                'Key to GraphIndexVocab.__getitem__ must be (iterable of) strings or iterable of integers')
