...
class PretrainedVocab(SetVocab):
    ...
    def __init__(self, pretrained_file=None, name=None, config=None):
        ...
        if (pretrained_file is None) != (name is None):
            raise ValueError(
                "You can't pass in a value for only one of pretrained_file and name to PretrainedVocab.__init__")
            ...
            if self.variable is None:
                with tf.device('/cpu:0'):
                    self.variable = tf.Variable(
                        self.embeddings, name=self.name + 'Embeddings', trainable=False)
                    tf.add_to_collection('non_save_variables', self.variable)
            layer = embeddings.pretrained_embedding_lookup(
                self.variable, self.linear_size, self.placeholder, name=self.name, reuse=reuse)
    ...
    def count(self, *args):
        ...
        with open_func(self.pretrained_file, 'rb') as f:
            ...
            shape[0] = min(
                shape[0],
                max_embed_count +
                cur_idx) if max_embed_count else shape[0]
            embeddings = np.zeros(shape, dtype=np.float32)

            # Fill in the embedding matrix
            # with open_func(self.pretrained_file, 'rt', encoding='utf-8') as
            # f:
            with open_func(self.pretrained_file, 'rb') as f:
    ...
    def dump(self):
        if self.save_as_pickle and not os.path.exists(self.vocab_loadname):
            ...
            with open(self.vocab_loadname, 'wb') as f:
                pkl.dump((self._tokens, self._embeddings),
    ...
    def load(self):
        ...
        with open(vocab_filename, 'rb') as f:
            self._tokens, self._embeddings = pkl.load(
