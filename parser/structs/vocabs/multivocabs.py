...
class Multivocab(BaseVocab, list):
    ...
    def __init__(self, config=None):
            ...
            if not pretrained_files:
                ...
                pretrained_vocabs = [
                    self._pretrained_vocab_class(
                        ...
                        config=config) for pretrained_file,
                    name in zip(
                        pretrained_files,
                        names)]
        ...
        for base_special_token in self[0].base_special_tokens:
            self.__dict__[
                base_special_token.upper() +
                '_STR'] = self[0].__dict__[
                base_special_token.upper() +
                '_STR']
            self.__dict__[
                base_special_token.upper() +
    ...
    def load(self):
        ...
        for vocab in self:
            status = (
                vocab.load() if hasattr(
                    vocab,
                    'load') else True) and status
        return status
    ...
    def count(self, train_conllus):
        ...
        status = True
        for vocab in self:
            status = (vocab._loaded or vocab.count(train_conllus)
                      if hasattr(vocab, 'count') else True) and status
        return status
        ...
        with tf.variable_scope(self.field):
            ...
            if self._pretrained_vocabs:
                with tf.variable_scope('Pretrained') as variable_scope:
                    input_tensors.extend(
                        [
                            pretrained_vocab.get_input_tensor(
                                embed_keep_prob=1.,
                                variable_scope=variable_scope,
                                reuse=reuse) for pretrained_vocab in self._pretrained_vocabs])
                    nonzero_init = False
            ...
            if self._elmo_vocabs:
                with tf.variable_scope('Elmo') as variable_scope:
                    input_tensors.extend(
                        [
                            elmo_vocab.get_input_tensor(
                                embed_keep_prob=1.,
                                variable_scope=variable_scope,
                                reuse=reuse) for elmo_vocab in self._elmo_vocabs])
                    nonzero_init = False

            if self._subtoken_vocab is not None:
                with tf.variable_scope('Subtoken') as variable_scope:
                    input_tensors.append(
                        self._subtoken_vocab.get_input_tensor(
            ...
            if self._token_vocab is not None:
                with tf.variable_scope('Token') as variable_scope:
                    input_tensors.append(
                        self._token_vocab.get_input_tensor(
                            nonzero_init=nonzero_init,
                            embed_keep_prob=1.,
                            variable_scope=variable_scope,
                            reuse=reuse))

            layer = self.combine_func(
                input_tensors,
        ...
        if hasattr(embeddings, combine_func):
            ...
            raise AttributeError(
                "module '{}' has no attribute '{}'".format(
                    embeddings.__name__, combine_func))
