...
class BaseVocab(object):
    ...
    def __init__(self, placeholder_shape=[None, None], config=None):
        ...
        self.placeholder = tf.placeholder(
            tf.int32, placeholder_shape, name=self.classname)
        self._config = config
        return
    ...
    def get_root(self):
        raise NotImplementedError(
            'get_root not implemented for %s' %
            self.classname)
...
class SetVocab(BaseVocab):
    ...
    def __init__(self, *args, **kwargs):
        ...
        special_tokens = [
            getattr(
                ...
                self._config.getstr(
                    self,
                    'special_token_case'))() for base_special_token in self._base_special_tokens]
        if self._config.getboolean(self, 'special_token_html'):
            special_tokens = [
                u'<%s>' %
                special_token for special_token in special_tokens]
    ...
    def __getitem__(self, key):
        if isinstance(key, six.string_types):
            ...
            raise ValueError(
                'key to SetVocab.__getitem__ must be (iterable of) string or integer')
        return
    ...
    def __setitem__(self, key, value):
        if isinstance(key, six.string_types):
            ...
            raise ValueError(
                'keys and values to SetVocab.__setitem__ must be (iterables of) string or integer')
    ...
    def __contains__(self, key):
        if isinstance(key, six.string_types):
            ...
            raise ValueError(
                'key to SetVocab.__contains__ must be string or integer')
        return
