...
class SubtokenVocab(CountVocab):
    ...
    def __init__(self, config=None):
        ...
        self._multibucket = ListMultibucket(
            self, max_buckets=self.max_buckets, config=config)
        self._tok2idx = {}
        self._idx2tok = {}
        return
    ...
    def get_input_tensor(
        ...
        with tf.variable_scope(variable_scope or self.classname) as scope:
            for i, placeholder in enumerate(
                ...
                with tf.variable_scope('Embeddings'):
                    layer = embeddings.token_embedding_lookup(
                        len(self), self.embed_size, placeholder, nonzero_init=True, reuse=reuse)

                seq_lengths = tf.count_nonzero(
                    placeholder, axis=1, dtype=tf.int32)
                ...
                if not self.squeeze_type.startswith('gated'):
                    ...
                    with tf.variable_scope('Attention'):
                        attn, layer = classifiers.deep_linear_attention(
                            layer, self.output_size, hidden_func=nonlin.identity, hidden_keep_prob=output_keep_prob)
                    if self.squeeze_type == 'gated_reduce_max':
                        layer = tf.nn.relu(tf.reduce_max(layer,
                                                         axis=-2)) + .1 * tf.reduce_sum(layer,
                                                                                        axis=-2) / (tf.count_nonzero(layer,
                                                                                                                     axis=-2,
            ...
            layer = tf.nn.embedding_lookup(
                layer, self._multibucket.placeholder)
    ...
    def add(self, token):
        ...
        character_indices = [self._str2idx.get(
            character, self.UNK_IDX) for character in characters[:50]]
