...
class TokenVocab(CountVocab):
    ...
    def get_input_tensor(
        ...
        embed_keep_prob = 1 if reuse else (
            embed_keep_prob or self.embed_keep_prob)

        with tf.variable_scope(variable_scope or self.classname):
            layer = embeddings.token_embedding_lookup(
                len(self), self.embed_size, inputs, nonzero_init=nonzero_init, reuse=reuse)
    ...
    def get_output_tensor(self, predictions, reuse=True):
        ...
        with tf.variable_scope(self.classname):
            layer = embeddings.token_embedding_lookup(
                len(self), self.embed_size, predictions, reuse=reuse)
            if embed_keep_prob < 1:
                layer = self.drop_func(layer, embed_keep_prob)
        return layer
        ...
        with tf.variable_scope(variable_scope or self.classname):
            ...
            with tf.variable_scope('Classifier'):
                logits = classifiers.linear_classifier(
                    layer, len(self), hidden_keep_prob=hidden_keep_prob)
    ...
    def get_sampled_linear_classifier(
        ...
        with tf.variable_scope(variable_scope or self.classname):
            ...
            layer = nn.dropout(
                layer, hidden_keep_prob, noise_shape=[
            ...
            with tf.variable_scope('Classifier'):
                ...
                with tf.device('/gpu:1'):
                    ...
                    biases = tf.get_variable(
                        'Biases', shape=len(self), initializer=tf.zeros_initializer)
                    ...
                    # (nm x s)
                    sample_weights = tf.to_float(
                        nn.not_equal(samples, targets))
                    # (nm x 1+s)
                    cands = tf.stop_gradient(
                        tf.concat([targets, samples], axis=-1))
                    # (nm x 1), (nm x s) -> (nm x 1+s)
                    cand_weights = tf.stop_gradient(tf.concat(
                        [nn.ones([batch_size * bucket_size, 1]), sample_weights], axis=-1))
                    ...
                    # (nm x 1+s)
                    logits = logits - \
                        tf.reduce_max(logits, axis=-1, keep_dims=True)
                    # (nm x 1+s)
                    exp_logits = tf.exp(logits) * cand_weights
                    # (nm x 1)
                    exp_logit_sum = tf.reduce_sum(
                        exp_logits, axis=-1, keep_dims=True)
                    # (nm x 1+s)
                    probabilities = exp_logits / exp_logit_sum
                    # (nm x 1+s) -> (n x m x 1+s)
                    probabilities = nn.reshape(
                        probabilities, [
                            batch_size, bucket_size, 1 + n_samples])
                    # (nm x 1+s) -> (n x m x 1+s)
                    samples = nn.reshape(
                        samples, [
                            batch_size, bucket_size, 1 + n_samples])
                    ...
                    # (n x m) -> (nm x 1)
                    token_weights1D = tf.to_float(
                        nn.reshape(token_weights, [-1, 1]))
                    # (nm x 1) -> ()
                    loss = tf.reduce_sum(
                        loss * token_weights1D) / tf.reduce_sum(token_weights1D)
                    ...
                    tokens_per_sequence = tf.reduce_sum(token_weights, axis=-1)
                    # (n x m) -> (n)
                    correct_tokens_per_sequence = tf.reduce_sum(
                        correct_tokens, axis=-1)
                    # (n), (n) -> (n)
                    correct_sequences = nn.equal(
                        tokens_per_sequence, correct_tokens_per_sequence)
                    ...
                    outputs['recur_layer'] = recur_layer
                    outputs['targets'] = targets
                    outputs['probabilities'] = tf.tuple(
                        [samples, probabilities])
                    outputs['loss'] = loss

                    outputs['predictions'] = predictions
                    outputs['n_correct_tokens'] = tf.reduce_sum(correct_tokens)
                    outputs['n_correct_sequences'] = tf.reduce_sum(
                        correct_sequences)
        return outputs

    # =============================================================
    def get_bilinear_classifier(
            self,
            layer,
            outputs,
            token_weights,
        ...
        with tf.variable_scope(variable_scope or self.classname):
            ...
            with tf.variable_scope('Classifier'):
                ...
                # (n x m) -> (n x m x m)
                unlabeled_predictions = tf.one_hot(
                    unlabeled_predictions, bucket_size)
                unlabeled_targets = tf.one_hot(unlabeled_targets, bucket_size)
                # (n x m x m) -> (n x m x m x 1)
                unlabeled_predictions = tf.expand_dims(
                    unlabeled_predictions, axis=-1)
                ...
                # Compute probabilities/cross entropy
                # (n x m x m) -> (n x m x m x 1)
                head_probabilities = tf.expand_dims(
                    tf.stop_gradient(outputs['probabilities']), axis=-1)
                ...
                label_loss = tf.losses.sparse_softmax_cross_entropy(
                    label_targets, oracle_logits, weights=token_weights)

                # -------------------------------------------------------
                # Compute predictions/accuracy
                # (n x m x c) -> (n x m)
                label_predictions = tf.argmax(
                    predicted_logits, axis=-1, output_type=tf.int32)
                label_oracle_predictions = tf.argmax(
                    oracle_logits, axis=-1, output_type=tf.int32)
                # (n x m) (*) (n x m) -> (n x m)
                correct_label_tokens = nn.equal(
                    label_targets, label_oracle_predictions) * token_weights
                correct_tokens = nn.equal(
                    label_targets, label_predictions) * outputs['correct_unlabeled_tokens']

                # (n x m) -> (n)
                tokens_per_sequence = tf.reduce_sum(token_weights, axis=-1)
                # (n x m) -> (n)
                correct_label_tokens_per_sequence = tf.reduce_sum(
                    correct_label_tokens, axis=-1)
                correct_tokens_per_sequence = tf.reduce_sum(
                    correct_tokens, axis=-1)
                # (n), (n) -> (n)
                correct_label_sequences = nn.equal(
                    tokens_per_sequence, correct_label_tokens_per_sequence)
                correct_sequences = nn.equal(
                    tokens_per_sequence, correct_tokens_per_sequence)
        ...
        # This way we can reconstruct the head_probabilities by exponentiating
        # and summing along the last axis
        ...
        outputs['n_correct_label_sequences'] = tf.reduce_sum(
            correct_label_sequences)
        outputs['n_correct_tokens'] = tf.reduce_sum(correct_tokens)
        outputs['n_correct_sequences'] = tf.reduce_sum(correct_sequences)
    ...
    # =============================================================
    def get_bilinear_classifier_with_embeddings(
            self,
            layer,
            embeddings,
            token_weights,
    ...
    def get_unfactored_bilinear_classifier(
        ...
        with tf.variable_scope(variable_scope or self.classname):
            ...
            with tf.variable_scope('Classifier'):
                ...
                reshaped_logits = tf.reshape(logits, tf.stack(
                    [-1, bucket_size, bucket_size * len(self)]))
                ...
                probabilities = tf.reshape(probabilities, tf.stack(
                    [-1, bucket_size, len(self), bucket_size]))
                # (n x m x c x m) -> (n x m x m x c)
                ...
                # (n x m x cm) -> (n x m)
                predictions = tf.argmax(
                    reshaped_logits, axis=-1, output_type=tf.int32)
                # (n x m), () -> (n x m)
                unlabeled_predictions = tf.mod(predictions, bucket_size)
                # (n x m) (*) (n x m) -> (n x m)
                correct_tokens = nn.equal(predictions, targets) * token_weights
                correct_unlabeled_tokens = nn.equal(
                    unlabeled_predictions, unlabeled_targets) * token_weights
                ...
                correct_tokens_per_sequence = tf.reduce_sum(
                    correct_tokens, axis=-1)
                correct_unlabeled_tokens_per_sequence = tf.reduce_sum(
                    correct_unlabeled_tokens, axis=-1)
                # (n), (n) -> (n)
                correct_sequences = nn.equal(
                    tokens_per_sequence, correct_tokens_per_sequence)
                correct_unlabeled_sequences = nn.equal(
                    tokens_per_sequence, correct_unlabeled_tokens_per_sequence)
        ...
        outputs['n_correct_unlabeled_tokens'] = tf.reduce_sum(
            correct_unlabeled_tokens)
            ...
            with codecs.open(train_conllu, encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    if line and not line.startswith('#'):
                        line = line.split('\t')
                        # conllu_idx is provided by the CoNLLUVocab
                        token = line[self.conllu_idx]
                        self._count(token)
...
class GraphTokenVocab(TokenVocab):
    ...
    # =============================================================
    def get_bilinear_discriminator(
            self,
            layer,
            token_weights,
            variable_scope=None,
        ...
        with tf.variable_scope(variable_scope or self.classname):
            ...
            with tf.variable_scope('Discriminator'):
                ...
                predictions = nn.greater(
                    logits, 0, dtype=tf.int32) * token_weights
                ...
                # (n x m x m) -> (n)
                n_targets_per_sequence = tf.reduce_sum(
                    unlabeled_targets, axis=[1, 2])
                n_true_positives_per_sequence = tf.reduce_sum(
                    true_positives, axis=[1, 2])
                # (n) x 2 -> ()
                n_correct_sequences = tf.reduce_sum(
                    nn.equal(
                        n_true_positives_per_sequence,
    ...
    def get_bilinear_classifier(
        ...
        head_probabilities = tf.expand_dims(
            tf.stop_gradient(outputs['probabilities']), axis=-1)
        # (n x m x m x c) -> (n x m x m x c)
        label_probabilities = tf.nn.softmax(
            transposed_logits) * tf.to_float(tf.expand_dims(token_weights, axis=-1))
        # (n x m x m), (n x m x m x c), (n x m x m) -> ()
        label_loss = tf.losses.sparse_softmax_cross_entropy(
            label_targets, transposed_logits, weights=token_weights * unlabeled_targets)
        ...
        # (n x m x m) (*) (n x m x m) -> (n x m x m)
        true_positives = nn.equal(
            label_targets,
            predictions) * unlabeled_predictions
        correct_label_tokens = nn.equal(
            label_targets, predictions) * unlabeled_targets
        # (n x m x m) -> ()
        ...
        n_true_positives_per_sequence = tf.reduce_sum(
            true_positives, axis=[1, 2])
        n_correct_label_tokens_per_sequence = tf.reduce_sum(
            correct_label_tokens, axis=[1, 2])
        # (n) x 2 -> ()
        n_correct_sequences = tf.reduce_sum(
            nn.equal(
                n_true_positives_per_sequence,
        ...
        n_correct_label_sequences = tf.reduce_sum(
            nn.equal(
                n_correct_label_tokens_per_sequence,
                n_targets_per_sequence))
    ...
    def get_unfactored_bilinear_classifier(
        ...
        with tf.variable_scope(variable_scope or self.field):
            ...
            with tf.variable_scope('Classifier'):
                ...
                n_targets_per_sequence = tf.reduce_sum(
                    unlabeled_targets, axis=[1, 2])
                n_unlabeled_true_positives_per_sequence = tf.reduce_sum(
                    unlabeled_true_positives, axis=[1, 2])
                n_true_positives_per_sequence = tf.reduce_sum(
                    true_positives, axis=[1, 2])
                # (n) x 2 -> ()
    ...
    def __getitem__(self, key):
        if isinstance(key, six.string_types):
                ...
                for edge in token:
                    ...
                    nodes.append(
                        (int(head),
                         super(
                            GraphTokenVocab,
                            self).__getitem__(rel)))
            return nodes
        elif hasattr(key, '__iter__'):
            if len(key) > 0 and hasattr(key[0], '__iter__'):
                if len(key[0]) > 0 and isinstance(
                        key[0][0], six.integer_types + (np.int32, np.int64)):
                    nodes = []
                    for (head, rel) in key:
                        nodes.append(
                            '{}:{}'.format(
            ...
            raise ValueError(
                'key to GraphTokenVocab.__getitem__ must be (iterable of) strings or iterable of integers')
