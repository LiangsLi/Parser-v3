...
class ElmoVocab(SetVocab):
    ...
    def __init__(self, config=None):
        ...
        if self._elmo_test_filename:
            ...
            self._elmo_train_filename = config.getstr(
                self, 'elmo_train_filename')
            ...
            if self.variable is None:
                with tf.device('/cpu:0'):
                    self.variable = tf.Variable(
                        self.embeddings, name=self.name + 'Elmo', trainable=False)
                    tf.add_to_collection('non_save_variables', self.variable)
            layer = embeddings.pretrained_embedding_lookup(
                self.variable,
                self.linear_size,
                self.placeholder,
                name=self.name,
    ...
    def count(self, *args):
        ...
        if self._elmo_test_filename:
            print(
                "### Loading ELMo for testset from {}! ###".format(
                    self._elmo_test_filename))
            with h5py.File(self._elmo_test_filename, 'r') as f:
                for sid, sent in enumerate(
                        self.iter_sents(self._conllu_files)):
                    sent_ = '\t'.join(sent)
                    sent_ = sent_.replace(
                        '/',
                        '$backslash$').replace(
            ...
            if self._elmo_train_filename:
                print(
                    "### Loading ELMo for trainset from {}! ###".format(
                        self._elmo_train_filename))
                with h5py.File(self._elmo_train_filename, 'r') as f:
                    for sid, sent in enumerate(
                            self.iter_sents(self._train_conllus)):
                        sent_ = '\t'.join(sent)
                        sent_ = sent_.replace(
                            '/',
                            '$backslash$').replace(
                        ...
                        for wid in range(len(sent)):
                            self["trainset-" +
                                 str(sid) + "-" + str(wid)] = cur_idx
                            cur_idx += 1

            if self._elmo_dev_filename:
                print(
                    "### Loading ELMo for devset from {}! ###".format(
                        self._elmo_dev_filename))
                with h5py.File(self._elmo_dev_filename, 'r') as f:
                    for sid, sent in enumerate(
                            self.iter_sents(self._dev_conllus)):
                        sent_ = '\t'.join(sent)
                        sent_ = sent_.replace(
                            '/',
                            '$backslash$').replace(
        ...
        try:
            ...
            embeddings = np.pad(
                embeddings, ((len(self.special_tokens), 0), (0, 0)), 'constant')
            ...
            raise ValueError(
                "Couldn't stack embeddings with shapes in %s" %
                shapes)
