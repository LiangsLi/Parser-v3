...
class CoNLLUDataset(set):
    ...
    def __init__(self, conllu_files, vocabs, config=None):
        ...
        self._multibucket = DictMultibucket(
            vocabs, max_buckets=config.getint(
                self, 'max_buckets'), config=config)
        self._is_open = False
        self._config = config
        self._conllu_files = conllu_files
        assert len(
            conllu_files) > 0, "You didn't pass in any valid CoNLLU files! Maybe you got the path wrong?"
    ...
    def load_next(self, file_idx=None):
        ...
        if self._cur_file_idx == -1 or len(self.conllu_files) > 1:
            ...
            if file_idx is None:
                self._cur_file_idx = (
                    self._cur_file_idx + 1) % len(self.conllu_files)
                file_idx = self._cur_file_idx

            with self.open():
                for sid, sent in enumerate(
                    self.itersents(
                        self.conllu_files[file_idx])):
    ...
    def add(self, sent, sid):
        ...
        for vocab in self:
            ...
            if 'FormMultivocab' in vocab.__class__.__name__:
                ...
                # for graphs, list of (head, label) pairs
    ...
    def batch_iterator(self, shuffle=False):
        ...
        for i in np.unique(bucket_indices):
            ...
            if len(subdata) > 0:
                ...
                n_splits = max(
                    subdata.shape[0] *
                    self._multibucket.max_lengths[i] //
                    self.batch_size,
                    1)
    ...
    def get_tokens(self, indices):
        ...
        token_dict = {}
        for vocab in self:
            token_dict[vocab.field] = self._multibucket.get_tokens(
                vocab.classname, indices)
