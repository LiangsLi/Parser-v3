...
class DictMultibucket(BaseMultibucket, dict):
    ...
    def __init__(self, vocabs, max_buckets=2, config=None):
        ...
        for vocab in vocabs:
            self[vocab.classname] = [DictBucket(
                idx, vocab.depth, config=config) for idx in six.moves.range(max_buckets)]
    ...
    def close(self):
        ...
        self._max_lengths = self.compute_max_lengths(
            self._lengths, self.max_buckets)
        len2bkt = self.get_len2bkt(self._max_lengths)

        # Open the buckets
        shape = len(self._lengths)
        dtype = [('bucket', 'i4')] + [(vocab_classname, 'i4')
                                      for vocab_classname in self.keys()]
        ...
        for i, vocab_classname in enumerate(self.keys()):
            ...
            for j, (indices, tokens) in enumerate(
                    zip(self._indices[vocab_classname], self._tokens[vocab_classname])):
                bucket_index = len2bkt[len(indices)]
                sequence_index = self[vocab_classname][bucket_index].add(
                    indices, tokens)
    ...
    def get_data(self, vocab_classname, indices):
        ...
        assert len(
            bucket_index) == 1, 'Requested data from multiple (or no) buckets'
