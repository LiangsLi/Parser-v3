...
class GraphOutputs(object):
    ...
    def __init__(
        ...
        self._loss = tf.add_n([tf.where(tf.is_finite(
            output['loss']), output['loss'], 0.) for output in outputs.values()])
    ...
    def probs_to_preds(self, probabilities, lengths):
        ...
        if 'form' in probabilities:
            ...
            if isinstance(form_probs, (tuple, list)):
                ...
                predictions['form'] = form_samples[np.arange(
                    len(form_preds)), form_preds]
            else:
                form_preds = np.argmax(form_probs, axis=-1)
        ...
        if 'xpos' in probabilities:
            ...
            if isinstance(xpos_probs, (tuple, list)):
                xpos_preds = np.concatenate([np.argmax(
                    xpos_prob_mat, axis=-1)[:, :, None] for xpos_prob_mat in xpos_probs], axis=-1)
            else:
        ...
        if 'ufeats' in probabilities:
            ...
            ufeats_preds = np.concatenate([np.argmax(
                ufeats_prob_mat, axis=-1)[:, :, None] for ufeats_prob_mat in ufeats_probs], axis=-1)
        ...
        if 'deptree' in probabilities:
            ...
            if self._factored_deptree:
                ...
                dephead_preds = np.zeros(
                    dephead_probs.shape[:2], dtype=np.int32)
                for i, (_dephead_probs, length) in enumerate(
                        zip(dephead_probs, lengths)):
                    # print(_dephead_probs)
                    # input()
                    cle = chuliu_edmonds_one_root(
                        _dephead_probs[:length, :length])
                    dephead_preds[i, :length] = cle
                # ()
                bucket_size = dephead_preds.shape[1]
                # (n x m) -> (n x m x m)
                one_hot_dephead_preds = (np.arange(
                    bucket_size) == dephead_preds[..., None]).astype(int)
                # (n x m x m) * (n x m x m x c) -> (n x m x c)
                deprel_probs = np.einsum(
                    'ijk,ijkl->ijl', one_hot_dephead_preds, deptree_probs)
                # (n x m x c) -> (n x m)
                ...
                deptree_probs = deptree_probs.reshape(
                    [-1, bucket_size, bucket_size * n_classes])
                # (n x m x mc) -> (n x m)
            ...
            for i in range(len(semgraph_preds)):
                ...
                for j in range(len(semgraph_preds[i])):
                    ...
                    for k, pred in enumerate(semgraph_preds[i, j]):
                        if pred:
                            sparse_semgraph_preds[-1][-1].append(
                                (k, semgraph_preds[i, j, k]))
        return predictions
    ...
    def sem16decoder(self, semgraph_probs, lengths):
        ...
        for i, (sem_preds, length) in enumerate(zip(semhead_preds, lengths)):
            masked_semhead_preds[i, :length,
                                 :length] = sem_preds[:length, :length]
        n_counts = {
            'no_root': 0,
            'multi_root': 0,
            'no_head': 0,
        ...
        for i, length in enumerate(lengths):
            ...
            for j, n_head in enumerate(n_heads):
                if n_head == 0:
                    ...
                    #print ('new graph:\n',masked_semhead_preds[i])
        print('Corrected List:', '\t'.join(
            [key + ':' + str(val) for key, val in n_counts.items()]))
        # (n x m x m x c) -> (n x m x m)
        semrel_preds = np.argmax(semgraph_probs, axis=-1)
        # (n x m x m) (*) (n x m x m) -> (n x m x m)
    ...
    def compute_token_accuracy(self, field):
        ...
        return self.history[field]['tokens'][-1] / \
            (self.history['total']['n_tokens'] + 1e-12)
    ...
    def compute_sequence_accuracy(self, field):
        ...
        return self.history[field]['sequences'][-1] / \
            self.history['total']['n_sequences']
        ...
        for field in self.history:
            if field in self.evals:
                if field.startswith('sem'):
                    token_accuracy += np.log(
                        self.compute_token_F1(field) + 1e-12)
                else:
                    token_accuracy += np.log(
                        self.compute_token_accuracy(field) + 1e-12)
        ...
        for field, output in six.iteritems(outputs):
            if field == 'semgraph':
                if self._factored_semgraph:
                    ...
                    self.history['semrel']['n_edges'] += output['n_true_positives'] + \
                        output['n_false_negatives']
        ...
        for field, string in self._print_mapping:
            if field in self.history:
                ...
                if field in ('semgraph', 'semhead'):
                    ...
                    self.history[field]['tokens'][-1] = self.compute_token_F1(
                        field) * 100
                elif field == 'semrel':
