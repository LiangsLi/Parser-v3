...
class GraphOutputs(object):
    ...
    def __init__(self, outputs, tokens, load=False, evals=None, factored_deptree=None, factored_semgraph=None,
        ...
        self._loss = tf.add_n(
            [tf.where(tf.is_finite(output['loss']), output['loss'], 0.) for output in outputs.values()])
    ...
    def probs_to_preds(self, probabilities, lengths):
        ...
        if 'ufeats' in probabilities:
            ...
            ufeats_preds = np.concatenate(
                [np.argmax(ufeats_prob_mat, axis=-1)[:, :, None] for ufeats_prob_mat in ufeats_probs], axis=-1)
