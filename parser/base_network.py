...
class BaseNetwork(object):
                ...
                try:
                    ...
                    while (not self.max_steps or current_step < self.max_steps) and \
                        ...
                        for batch in trainset.batch_iterator(shuffle=True):
                            ...
                            # ---
                            if current_step < 10:
                                _, train_scores = sess.run(train_tensors, feed_dict=feed_dict, options=options,
            ...
            if len(conllu_files) == 1 or output_filename is not None:
                with Timer('Parsing file'):
                    if self.other_save_dirs is None:
                        ...
                        self.parse_file_ensemble(parseset, parse_outputs, sess, saver, output_dir=output_dir,
                                                 output_filename=output_filename)
            else:
    ...
    # =============================================================
    def parse_file_ensemble(self, dataset, graph_outputs, sess, saver, output_dir=None, output_filename=None,
                            print_time=True):
        """"""
