...
class BaseNetwork(object):
    ...
    def __init__(self, input_networks=set(), config=None):
        ...
        with Timer('Initializing the network (including pretrained vocab)'):
            ...
            input_network_classes = set(
                input_network.classname for input_network in self._input_networks)
            assert input_network_classes == set(
                self.input_network_classes), 'Not all input networks were passed in to {}'.format(
                self.classname)
                ...
                for vocab in input_network.vocabs:
                    if vocab.classname in extant_vocabs:
                        assert vocab is extant_vocabs[
                            vocab.classname], "Two input networks have different instances of {}".format(
                            vocab.classname)
                    else:
    ...
    def train(self, load=False, noscreen=False):
        ...
        for input_network in self.input_networks:
            with tf.variable_scope(input_network.classname, reuse=False):
                input_network_outputs[input_network.classname] = input_network.build_graph(reuse=True)[
                    0]
            network_variables = set(
                tf.global_variables(
                    scope=input_network.classname))
        ...
        with tf.variable_scope(self.classname, reuse=False):
            train_graph = self.build_graph(
                input_network_outputs=input_network_outputs, reuse=False)
        ...
        with tf.variable_scope(self.classname, reuse=True):
            dev_graph = self.build_graph(
                input_network_outputs=input_network_outputs, reuse=True)
            dev_outputs = DevOutputs(
                *dev_graph,
                load=load,
                evals=self._evals,
                factored_deptree=factored_deptree,
                factored_semgraph=factored_semgraph,
                config=self._config)
        regularization_loss = self.l2_reg * \
            tf.losses.get_regularization_loss() if self.l2_reg else 0

        update_step = tf.assign_add(self.global_step, 1)
        adam = AdamOptimizer(config=self._config)
        adam_op = adam.minimize(
            train_outputs.loss + regularization_loss,
        ...
        with tf.Session(config=config) as sess:
            ...
            if not noscreen:
                ...
                def run(stdscr):
                    ...
                    stdscr.addstr('\t')
                    stdscr.addstr(
                        'Moving acc: {:5.2f}'.format(0.),
                        curses.color_pair(1) | curses.A_BOLD)
                    stdscr.addstr(' | ')
                    stdscr.addstr(
                        'Best moving acc: {:5.2f}\n'.format(0.),
                        curses.color_pair(1) | curses.A_BOLD)
                    stdscr.clrtoeol()
                    stdscr.addstr('\t')
                    stdscr.addstr(
                        'Steps since improvement: {:4d}\n'.format(0),
                        curses.color_pair(1) | curses.A_BOLD)
                    stdscr.clrtoeol()
                    stdscr.move(2, 0)
                    stdscr.refresh()
                    try:
                        ...
                        steps_since_best = 0
                        while (
                            not self.max_steps or current_step < self.max_steps) and (
                            not self.max_steps_without_improvement or steps_since_best < self.max_steps_without_improvement) and (
                            not self.n_passes or current_epoch < len(
                                trainset.conllu_files) * self.n_passes):
                            ...
                            for batch in trainset.batch_iterator(shuffle=True):
                                ...
                                _, train_scores = sess.run(
                                    train_tensors, feed_dict=feed_dict)
                                train_outputs.update_history(train_scores)
                                current_step += 1
                                if current_step % self.print_every == 0:
                                    for batch in devset.batch_iterator(
                                            shuffle=False):
                                        dev_outputs.restart_timer()
                                        feed_dict = devset.set_placeholders(
                                            batch)
                                        dev_scores = sess.run(
                                            dev_tensors, feed_dict=feed_dict)
                                    ...
                                    if current_accuracy >= best_accuracy:
                                        ...
                                        if self.parse_devset:
                                            self.parse_files(
                                                devset, dev_outputs, sess, print_time=False)
                                    else:
                                    ...
                                    stdscr.addstr('\t')
                                    stdscr.addstr(
                                        'Moving acc: {:5.2f}'.format(current_accuracy),
                                        curses.color_pair(1) | curses.A_BOLD)
                                    stdscr.addstr(' | ')
                                    stdscr.addstr(
                ...
                try:
                    ...
                    while (
                        ...
                        for batch in trainset.batch_iterator(shuffle=True):
                            ...
                            if current_step < 10:
                                _, train_scores = sess.run(
                                    train_tensors, feed_dict=feed_dict, options=options, run_metadata=run_metadata)
                                fetched_timeline = timeline.Timeline(
                                    run_metadata.step_stats)
                                chrome_trace = fetched_timeline.generate_chrome_trace_format()
                                with open(
                                        os.path.join(self.save_dir, 'profile', 'timeline_step_%d.json' % current_step),
                                        'w') as f:
                            ...
                            if current_step % self.print_every == 0:
                                ...
                                print(
                                    'Epoch: {:3d}'.format(
                                        int(current_epoch)), end='')
                                print(' | ', end='')
                                print(
                                    'Step: {:5d}\n'.format(
                                        int(current_step)), end='')
                                print('\t', end='')
                                print(
                                    'Moving acc: {:5.2f}'.format(current_accuracy), end='')
                                print(' | ', end='')
                                print(
    ...
    def parse(self, conllu_files, output_dir=None, output_filename=None):
        ...
        if output_filename:
            assert len(
                conllu_files) == 1, "output_filename can only be specified for one input file"
        factored_deptree = None
        factored_semgraph = None
        for vocab in self.output_vocabs:
            if vocab.field == 'deprel':
        ...
        with tf.Session(config=config) as sess:
            ...
            if len(conllu_files) == 1 or output_filename is not None:
                ...
                with Timer('Parsing files'):
                    self.parse_files(
                        parseset, parse_outputs, sess, output_dir=output_dir)
        return
    ...
    def parse_file(
        ...
        for i, indices in enumerate(dataset.batch_iterator(shuffle=False)):
            with Timer('Parsing batch %d' % i):
                ...
                probabilities = sess.run(
                    probability_tensors, feed_dict=feed_dict)
                predictions = graph_outputs.probs_to_preds(
                    probabilities, lengths)
                tokens.update(
                    {vocab.field: vocab[predictions[vocab.field]] for vocab in self.output_vocabs})
            ...
            if output_dir is None and output_filename is None:
                ...
                if output_dir is None:
                    output_dir = os.path.join(
                        self.save_dir, 'parsed', input_dir)
        ...
        if print_time:
            print(
                '\033[92mParsing 1 file took {:0.1f} seconds\033[0m'.format(
                    time.time() - graph_outputs.time))
        return

    # =============================================================
    def parse_file_ensemble(
            self,
            dataset,
            graph_outputs,
            sess,
        ...
        for i, collect in enumerate(collects):
            ...
            with Timer('Parsing batch %d' % i):
                predictions = graph_outputs.probs_to_preds(
                    collect['probs'], collect['lengths'])
                collect['tokens'].update(
                    {vocab.field: vocab[predictions[vocab.field]] for vocab in self.output_vocabs})
                graph_outputs.cache_predictions(
                    collect['tokens'], collect['indices'])
    ...
    def parse_files(
        ...
        if print_time:
            ...
            print('\033[92mParsing {} file{} took {:0.1f} seconds\033[0m'.format(
                n_files, 's' if n_files > 1 else '', time.time() - graph_outputs.time))
        return
