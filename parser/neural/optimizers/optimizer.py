...
class Optimizer(object):
    ...
    def minimize(self, loss, variables=None):
        ...
        gradients = {
            variable: gradient for variable,
            gradient in zip(
                variables,
                gradients) if gradient is not None}
    ...
    def dense_moving_average(
        ...
        iteration = self.get_accumulator(
            '{}/iteration'.format(name), variable, shape=[])
        iteration = tf.assign_add(iteration, 1)

        if decay < 1:
            current_decay = decay * \
                (1 - decay**(iteration - 1)) / (1 - decay**iteration)
        else:
            current_decay = (iteration - 1) / iteration

        accumulator = tf.assign(accumulator, current_decay * accumulator)
        accumulator = tf.assign_add(
            accumulator, (1 - current_decay) * accumulant)
    ...
    def sparse_moving_average(
        ...
        iteration = self.get_accumulator(
            '{}/iteration'.format(name),
            variable,
            shape=[
                first_dim,
                1])
        indexed_iteration = tf.gather(iteration, unique_indices)
        iteration = tf.scatter_add(
            iteration,
            unique_indices,
            tf.ones_like(indexed_iteration))
        indexed_iteration = tf.gather(iteration, unique_indices)

        if decay < 1:
            current_indexed_decay = decay * \
                (1 - decay**(indexed_iteration - 1)) / (1 - decay**indexed_iteration)
        else:
            current_indexed_decay = (indexed_iteration - 1) / indexed_iteration

        accumulator = tf.scatter_update(
            accumulator,
            unique_indices,
            current_indexed_decay *
            indexed_accumulator)
        accumulator = tf.scatter_add(
            accumulator,
            unique_indices,
            (1 - current_indexed_decay) * accumulant)
        return accumulator, iteration
    ...
    def clip_by_global_norm(self, variable_steps):
        ...
        variable_step_list, _ = tf.clip_by_global_norm(
            variable_step_list, self.clip)
                ...
                with tf.variable_scope(original_variable.op.name, reuse=False):
                    with tf.device(original_variable.device):
                        variable = tf.get_variable(
                            name, shape=shape, initializer=initializer, collections=[
                                tf.GraphKeys.GLOBAL_VARIABLES, 'non_save_variables'], trainable=False)
