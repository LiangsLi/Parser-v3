...
class AMSGradOptimizer(Optimizer):
    ...
    def dense_update(self, gradient, variable):
        ...
        if self.nu > 0:
            ...
            max_deviation = tf.assign(
                max_deviation, tf.maximum(
                    zero_deviation, max_deviation))
    ...
    def sparse_update(self, gradient, variable):
        ...
        indexed_gradient = tf.unsorted_segment_sum(
            gradient.values, sorting_indices, tf.size(unique_indices))
        if self.mu > 0:
            ...
            indexed_mean = (1 - self.gamma) * indexed_mean + \
                self.gamma * indexed_gradient
            updates.extend(mean_update)
        else:
        ...
        if self.nu > 0:
            ...
            zero_deviation_update = self.sparse_moving_average(
                ...
                decay=self.nu)
            zero_deviation, _ = zero_deviation_update
            indexed_max_deviation = tf.maximum(
                tf.gather(
                    max_deviation, unique_indices), tf.gather(
                    zero_deviation, unique_indices))
            max_deviation_update = tf.scatter_update(
                max_deviation, unique_indices, indexed_max_deviation)
            indexed_max_deviation = tf.sqrt(
                indexed_max_deviation + self.epsilon)
            updates.extend(zero_deviation_update)
            updates.append(max_deviation)
        else:
            indexed_max_deviation = 1

        indexed_variable_step = self.annealed_learning_rate * \
            indexed_mean / indexed_max_deviation
