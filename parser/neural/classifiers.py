    ...
    for i, hidden_size in enumerate(hidden_sizes):
        weights.append(
            tf.get_variable(
                ...
                shape=[
                    input_size,
                    hidden_size]))  # , initializer=tf.orthogonal_initializer))
...
def batch_bilinear_classifier(
    ...
    weights = tf.get_variable(
        ...
        initializer=tf.zeros_initializer)
    if hidden_keep_prob < 1.:
        noise_shape1 = tf.stack(
            layer_shape[:-1] + [1, input1_size - add_linear])
        noise_shape2 = tf.stack(
            layer_shape[:-1] + [1, input2_size - add_linear])
...
def bilinear_classifier(
    ...
    layer = nn.reshape(layer, layer_shape +
                       [output_size, bucket_size]) + biases
    return layer

# ===============================================================


def diagonal_bilinear_classifier(
        layer1,
        layer2,
        output_size,
        hidden_keep_prob=1.,
...
def diagonal_bilinear_discriminator(
        layer1,
        layer2,
        hidden_keep_prob=1.,
        add_linear=True):
...
def diagonal_bilinear_attention(
        layer1,
        layer2,
        hidden_keep_prob=1.,
        add_linear=True):
