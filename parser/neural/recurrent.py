...
def RNN(
    ...
    if conv_keep_prob < 1:
        layer = nn.dropout(
            layer, conv_keep_prob, noise_shape=[
    ...
    with tf.variable_scope('Loop'):
        ...
        weights = tf.get_variable(
            ...
            shape=[
                ...
                recur_size],
            initializer=tf.zeros_initializer)
        initial_hidden = tf.get_variable(
            'Initial_hidden', shape=[
                1, recur_size], initializer=tf.zeros_initializer)
        initial_hidden = nn.tile(initial_hidden, [batch_size, 1])
        null_hidden = tf.zeros_like(initial_hidden)
        mask = nn.drop_mask(
            tf.stack([batch_size, recur_size]), recur_keep_prob)
        ...
        def body(i, last_hidden, last_hidden_sequence):
            ...
            if recur_keep_prob < 1:
                last_hidden *= mask
            current_hidden = current_partial_input + \
                tf.matmul(last_hidden, weights)
            current_hidden = recur_func(current_hidden)
            if recur_include_prob < 1:
                zone_mask = nn.binary_mask(
                    [batch_size, recur_size], recur_include_prob)
                current_hidden = zone_mask * current_hidden + \
                    (1 - zone_mask) * last_hidden
            current_hidden = tf.where(
                i < seq_lengths, current_hidden, null_hidden)
            current_hidden_sequence = last_hidden_sequence.write(
                i, tf.where(i < seq_lengths, current_hidden, null_hidden))
...
def LSTM(
        ...
        seq_lengths,
        conv_width=0,
        recur_func=nonlin.tanh,
        conv_keep_prob=1.,
        recur_keep_prob=1.,
        recur_include_prob=1.,
        cifg=False,
        highway=False,
        highway_func=tf.identity):
    """"""
    ...
    if conv_size == 1:
        # , initializer=tf.orthogonal_initializer)
        weights = tf.get_variable('Weights', shape=[input_size, recur_size])
        # , initializer=tf.orthogonal_initializer)
        gate_weights = tf.get_variable(
            'Gate_Weights', shape=[
                input_size, gate_size])
        if highway:
            highway_weights = tf.get_variable(
                'Highway_Weights', shape=[
                    input_size, highway_size])  # , initializer=tf.orthogonal_initializer)
            # weights = tf.concat([weights, gate_weights, highway_weights], axis=2)
            weights = tf.concat(
                [weights, gate_weights, highway_weights], axis=1)
        else:
            # weights = tf.concat([weights, gate_weights], axis=2)
            weights = tf.concat([weights, gate_weights], axis=1)
        biases = tf.get_variable(
            'Biases',
            shape=gated_highway_size,
            initializer=tf.zeros_initializer)
        if conv_keep_prob < 1:
            layer = nn.dropout(
                layer, conv_keep_prob, noise_shape=[
        ...
        if highway:
            layer, highway_layer = tf.split(
                layer, [gated_size, highway_size], axis=2)
    else:
        # , initializer=tf.orthogonal_initializer)
        weights = tf.get_variable(
            'Weights', shape=[
                conv_size, input_size, recur_size])
        gate_weights = tf.get_variable(
            'Gate_Weights',
            shape=[
                conv_size,
                input_size,
                gate_size])  # , initializer=tf.orthogonal_initializer)
        if highway:
            highway_weights = tf.get_variable(
                'Highway_Weights',
                shape=[
                    conv_size,
                    input_size,
                    highway_size])  # , initializer=tf.orthogonal_initializer)
            weights = tf.concat(
                [weights, gate_weights, highway_weights], axis=2)
        else:
            weights = tf.concat([weights, gate_weights], axis=2)
        biases = tf.get_variable(
            'Biases',
    ...
    with tf.variable_scope('Loop'):
        ...
        # , initializer=tf.orthogonal_initializer) # TODO try zeros_initializer
        weights = tf.get_variable('Weights', shape=[recur_size, recur_size])
        # , initializer=tf.orthogonal_initializer)
        gate_weights = tf.get_variable(
            'Gate_Weights', shape=[
                recur_size, gate_size])
        weights = tf.concat([weights, gate_weights], axis=1)
        initial_state = tf.get_variable(
            'Initial_state', shape=[
                1, 2 * recur_size], initializer=tf.zeros_initializer)
        ...
        def body(i, last_state, last_state_sequence):
            ...
            if recur_keep_prob < 1:
                last_hidden *= mask
            current_input = current_partial_input + \
                tf.matmul(last_hidden, weights)
            current_hidden, current_cell = LSTM_func(
                current_input, last_cell, recur_func=recur_func, cifg=cifg)
            if recur_include_prob < 1:
                zone_mask = nn.binary_mask(
                    [batch_size, recur_size], recur_include_prob)
                current_hidden = zone_mask * current_hidden + \
                    (1 - zone_mask) * last_hidden
                current_cell = zone_mask * current_cell + \
                    (1 - zone_mask) * last_cell
            current_state = tf.concat([current_hidden, current_cell], 1)
            current_state_sequence = last_state_sequence.write(
                i, current_state)
...
def UniRNN(layer, recur_size, seq_lengths, recur_cell=LSTM, **kwargs):
    ...
    locations = tf.expand_dims(tf.one_hot(
        seq_lengths - 1, tf.shape(layer)[1]), -1)
    with tf.variable_scope('RNN'):
        hidden, cell = recur_cell(layer, recur_size, seq_lengths, **kwargs)
    layer = hidden
    if recur_cell == RNN:
        final_states = tf.squeeze(
            tf.matmul(hidden, locations, transpose_a=True), -1)
    else:
        final_hidden = tf.squeeze(
            tf.matmul(hidden, locations, transpose_a=True), -1)
        final_cell = tf.squeeze(
            tf.matmul(
                cell, locations, transpose_a=True), -1)
...
def BiRNN(
    ...
    with tf.variable_scope('RNN_FW'):
        fw_hidden, fw_cell = recur_cell(
            layer, recur_size, seq_lengths, **kwargs)
    rev_layer = tf.reverse_sequence(
        layer, seq_lengths, batch_axis=0, seq_axis=1)
    with tf.variable_scope('RNN_BW'):
        bw_hidden, bw_cell = recur_cell(
            rev_layer, recur_size, seq_lengths, **kwargs)
    rev_bw_hidden = tf.reverse_sequence(
        bw_hidden, seq_lengths, batch_axis=0, seq_axis=1)
    rev_bw_cell = tf.reverse_sequence(
        bw_cell, seq_lengths, batch_axis=0, seq_axis=1)
    if bilin:
        layer = tf.concat([fw_hidden * rev_bw_hidden,
                           fw_hidden, rev_bw_hidden], 2)
    else:
    ...
    if recur_cell == RNN:
        ...
        final_fw_hidden = tf.squeeze(
            tf.matmul(fw_hidden, locations, transpose_a=True), -1)
        final_fw_cell = tf.squeeze(
            tf.matmul(fw_cell, locations, transpose_a=True), -1)
        final_rev_bw_hidden = tf.squeeze(
            tf.matmul(rev_bw_hidden, locations, transpose_a=True), -1)
        final_rev_bw_cell = tf.squeeze(
            tf.matmul(rev_bw_cell, locations, transpose_a=True), -1)
        final_states = tf.concat(
            [final_fw_hidden, final_rev_bw_hidden, final_fw_cell, final_rev_bw_cell], 1)
    return layer, final_states
