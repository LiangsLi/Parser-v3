...
# ===============================================================
def RNN(layer, recur_size, conv_width=0, recur_func=nonlin.relu, conv_keep_prob=1., recur_keep_prob=1.,
        recur_include_prob=1., **kwargs):
    """"""
    ...
    weights = tf.get_variable('Weights',
                              shape=[conv_size, input_size, recur_size])  # , initializer=tf.orthogonal_initializer)
    ...
    with tf.variable_scope('Loop'):
        ...
        def body(i, last_hidden, last_hidden_sequence):
            ...
            current_hidden_sequence = last_hidden_sequence.write(i,
                                                                 tf.where(i < seq_lengths, current_hidden, null_hidden))
...
# ===============================================================
def LSTM(layer, recur_size, seq_lengths, conv_width=0, recur_func=nonlin.tanh, conv_keep_prob=1., recur_keep_prob=1.,
         recur_include_prob=1., cifg=False, highway=False, highway_func=tf.identity):
    """"""
    ...
    if conv_size == 1:
        ...
        gate_weights = tf.get_variable('Gate_Weights',
                                       shape=[input_size, gate_size])  # , initializer=tf.orthogonal_initializer)
        if highway:
            highway_weights = tf.get_variable('Highway_Weights', shape=[input_size,
        ...
        weights = tf.get_variable('Weights',
                                  shape=[conv_size, input_size, recur_size])  # , initializer=tf.orthogonal_initializer)
        gate_weights = tf.get_variable('Gate_Weights', shape=[conv_size, input_size,
                                                              gate_size])  # , initializer=tf.orthogonal_initializer)
        if highway:
            highway_weights = tf.get_variable('Highway_Weights', shape=[conv_size, input_size,
                                                                        highway_size])  # , initializer=tf.orthogonal_initializer)
            weights = tf.concat([weights, gate_weights, highway_weights], axis=2)
        else:
            weights = tf.concat([weights, gate_weights], axis=2)
    ...
    with tf.variable_scope('Loop'):
        ...
        weights = tf.get_variable('Weights', shape=[recur_size,
                                                    recur_size])  # , initializer=tf.orthogonal_initializer) # TODO try zeros_initializer
        gate_weights = tf.get_variable('Gate_Weights',
                                       shape=[recur_size, gate_size])  # , initializer=tf.orthogonal_initializer)
        weights = tf.concat([weights, gate_weights], axis=1)
