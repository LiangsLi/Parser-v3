...
def load_conllu(file):
    ...
    class UDRepresentation:
        def __init__(self):
            ...
            self.characters = []
            # List of UDSpan instances with start&end indices into
            # `characters`.
            self.tokens = []
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            # if root).
            self.parent = None
            # List of references to UDWord instances representing
            # functional-deprel children.
            self.functional_children = []
            # Only consider universal FEATS.
            self.columns[FEATS] = "|".join(
                sorted(
                    feat for feat in columns[FEATS].split("|") if feat.split(
                        "=", 1)[0] in UNIVERSAL_FEATURES))
            # Let's ignore language-specific deprel subtypes.
            self.columns[DEPREL] = columns[DEPREL].split(":")[0]
            # Precompute which deprels are CONTENT_DEPRELS and which
            # FUNCTIONAL_DEPRELS
    ...
    while True:
        ...
        if not line:
            # Add parent and children UDWord links and check there are no
            # cycles
            ...
            # because it is called recursively and may result in adding one
            # child twice.
            for word in ud.words[sentence_start:]:
            ...
            if len([word for word in ud.words[sentence_start:]
                    if word.parent is None]) != 1:
        ...
        columns[FORM] = "".join(
            filter(
                lambda c: unicodedata.category(c) != "Zs",
                columns[FORM]))
        if sys.version_info < (3, 0) and isinstance(line, str):
        ...
        if "-" in columns[ID]:
            ...
            for _ in range(start, end + 1):
                ...
                if len(word_columns) != 10:
                    raise UDError(
                        "The CoNLL-U line does not contain 10 tab-separated columns: '{}'".format(word_line))
                ud.words.append(
...
def evaluate(gold_ud, system_ud):
    class Score:
        def __init__(
            ...
            self.f1 = 2 * correct / \
                (system_total + gold_total) if system_total + gold_total else 0.0
    ...
    def alignment_score(alignment, key_fn=None, filter_fn=None):
        if filter_fn is not None:
            ...
            system = sum(
                1 for system in alignment.system_words if filter_fn(system))
            aligned = sum(
                1 for word in alignment.matched_words if filter_fn(
                    word.gold_word))
        else:
        ...
        def gold_aligned_system(word):
            return alignment.matched_words_map.get(
                word, "NotAligned") if word is not None else None
        correct = 0
        for words in alignment.matched_words:
            if filter_fn is None or filter_fn(words.gold_word):
                if key_fn(
                        words.gold_word,
                        gold_aligned_gold) == key_fn(
                        words.system_word,
                        gold_aligned_system):
                    correct += 1
    ...
    def find_multiword_span(gold_words, system_words, gi, si):
        ...
        while not beyond_end(gold_words, gi, multiword_span_end) or \
            ...
            if gi < len(gold_words) and (si >= len(
                    system_words) or gold_words[gi].span.start <= system_words[si].span.start):
                multiword_span_end = extend_end(
    ...
    def compute_lcs(gold_words, system_words, gi, si, gs, ss):
        ...
        for g in reversed(range(gi - gs)):
            for s in reversed(range(si - ss)):
                if lower(gold_words[gs + g].columns[FORM]
                         ) == lower(system_words[ss + s].columns[FORM]):
                    lcs[g][s] = 1 + (lcs[g + 1][s + 1] if g +
                                     1 < gi - gs and s + 1 < si - ss else 0)
                lcs[g][s] = max(lcs[g][s], lcs[g + 1][s]
                                if g + 1 < gi - gs else 0)
                lcs[g][s] = max(lcs[g][s], lcs[g][s + 1]
                                if s + 1 < si - ss else 0)
        return lcs
    ...
    def align_words(gold_words, system_words):
        ...
        while gi < len(gold_words) and si < len(system_words):
            if gold_words[gi].is_multiword or system_words[si].is_multiword:
                ...
                # "multiword span".
                gs, ss, gi, si = find_multiword_span(
                ...
                if si > ss and gi > gs:
                    ...
                    while g < gi - gs and s < si - ss:
                        if lower(
                                gold_words[gs + g].columns[FORM]) == lower(system_words[ss + s].columns[FORM]):
            ...
            if args.counts:
                print(
                    "{:11}|{:10} |{:10} |{:10} |{:10}".format(
                        ...
                        evaluation[metric].aligned_total or (
                            evaluation[metric].correct if metric == "Words" else "")))
            else:
                print(
                    "{:11}|{:10.2f} |{:10.2f} |{:10.2f} |{}".format(
                        ...
                        "{:10.2f}".format(
                            100 *
                            evaluation[metric].aligned_accuracy) if evaluation[metric].aligned_accuracy is not None else ""))
...
class TestAlignment(unittest.TestCase):
    ...
    def _test_exception(self, gold, system):
        self.assertRaises(
            UDError,
            evaluate,
            self._load_words(gold),
            self._load_words(system))

    def _test_ok(self, gold, system, correct):
        metrics = evaluate(self._load_words(gold), self._load_words(system))
        gold_words = sum((max(1, len(word.split(" ")) - 1) for word in gold))
        system_words = sum((max(1, len(word.split(" ")) - 1)
                            for word in system))
        self.assertEqual((metrics["Words"].precision,
                          metrics["Words"].recall,
                          metrics["Words"].f1),
