...
def chuliu_edmonds(scores):
    ...
    if not cycles:
        ...
        metanode_head_scores = scores[cycle][:,
                                             noncycle] - cycle_scores[:,
                                                                      None] + cycle_score
        ...
        # set the contracted graph scores of cycle's potential heads; (c x
        # n)[:, (n) in n] in R -> (n) in R
        subscores[-1,
                  :-1] = metanode_head_scores[metanode_heads,
                                              np.arange(len(noncycle_locs))]
        # set the contracted graph scores of cycle's potential dependents; (n x
        # c)[(n) in n] in R-> (n) in R
        subscores[:-1, - \
            1] = metanode_dep_scores[np.arange(len(noncycle_locs)), metanode_deps]
        ...
        # add the nodes to the new tree (t)[(n)[(n) in [0,1]] in t] in t =
        # (n)[(n)[(n) in [0,1]] in n] in t
        new_tree[noncycle_locs[contracted_subtree]
        ...
        # add the nodes to the tree (t)[(n)[(n) in [0,1]] in t] in t =
        # (c)[(n)[(n) in [0,1]] in c] in t
        new_tree[noncycle_locs[contracted_subtree]
                 ] = cycle_locs[metanode_deps[contracted_subtree]]
        #print(2, new_tree)
        # add the old cycle to the tree; (t)[(c) in t] in t = (t)[(c) in t] in
        # t
        new_tree[cycle_locs] = tree[cycle_locs]
        #print(3, new_tree)
        # root of the cycle; (n)[() in n] in c = () in c
        cycle_root = metanode_heads[cycle_head]
        # add the root of the cycle to the new tree; (t)[(c)[() in c] in t] =
        # (c)[() in c]
